...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("G")
    updateDisplayUnit();
}
...
{
    QString ret = QString("Gheez:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non G unit input in lnReqAmount, we generate the URI with G as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::G, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
